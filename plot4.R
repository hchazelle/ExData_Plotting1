...
#Open png device, create file and plots
dev.new(png)
dev.set(dev.list()["png"])
png(filename = "plot4.png", width = 480, height = 480)
par(mar= c(4,4,2,2),mfcol = c(2,2))
with(data = mydata, plot(datetime, Global_active_power, type = "l", ylab = "Global Active Power", xlab=""))
with(data = mydata, plot(datetime, Sub_metering_1, type = "l", ylab = "Energy sub metering", xlab=""))
...
with(data= mydata, plot(datetime, Voltage, type = "l"))
with(data= mydata, plot(datetime, Global_reactive_power, type = "l"))
dev.off()
